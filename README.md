...
- `result/`?`cache/`?`outputs/`
- `fw/`
- `config/`
- `plan.md`
- `.current_result_dir`?`.current_logger_pid`

## ??/??

??????? Polaris ????????? Codex skill ??????? skill ??????????

1. `environment-and-migration.md`
2. `capabilities-and-usage.md`
3. `SKILL.md`

## ?? Skill ??

- Skill name: `polaris-device-validation`
- Title: `Polaris 设备验证技能`
