...
当前本地 Runtime 还提供 adapter registry、capability matrix、event graph、state assertion DSL、Validation IR 和 analytics trend 入口，分别用于回答“这个项目能测什么、要占用什么资源、事件因果链是什么、最终状态是否安全、task/env 如何进入统一 IR、历史结果趋势如何”。
